                            ...
                            uint16_t datalen)
{
    ...
    if (message_count == NUM_NODES || etimer_expired(&aggr_timer))
    {
        ...
        int aggregated_data_int = aggregated_data;
        double aggregated_data_frac = (aggregated_data - aggregated_data_int) * 1000;

        printf("Aggregated data: %d.%03u, Missed: %d #\n", aggregated_data_int, (unsigned int)aggregated_data_frac, messages_missed);
